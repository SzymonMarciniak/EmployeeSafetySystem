...
from modules.global_vars import cameras_dict, choosenWorkplace
...
class SetupScreen(Screen):
    ...
    def show_ids(self, clear=False, show=True):
        ...
        if (self.ids["ShowIDButton"].state == "down") and show:
            ...
            for nr, room in enumerate(rooms_list):
                print(room.floor, self.current_floor)
                if room.floor == self.current_floor:
        ...
        if my_id:
            ...
            for nr, camera in enumerate(cameras_list):
                if camera.deleteId == int(my_id):
                    ...
                    except: print("Object doest exist")
    ...
    def set_object_name(self, popup):
        ...
        if new_created == "camera":
            ...
            db, cursor = connectToDatabase()
            cursor.execute(f"""INSERT INTO rooms VALUES (null, {new_points[0]}, {new_points[1]}, {new_points[2]}, {new_points[3]}, '{new_name}' ,{new_id}, {self.current_floor}, {global_vars.choosenWorkplace})""")
            db.commit()
            closeDatabaseConnection(db, cursor)
            self.rooms_refresh()
            ...
            if self.current_floor != None:
                ...
                if self.choose == "create_door":
                    ...
                    if self.to_create:
                        ...
                        db, cursor = connectToDatabase()
                        cursor.execute(f"""INSERT INTO doors VALUES (null, {self.kx1}, {self.ky1}, {self.kx2}, {self.ky2} ,{new_id}, {self.current_floor}, {global_vars.choosenWorkplace})""")
                        db.commit()
                        closeDatabaseConnection(db, cursor)
