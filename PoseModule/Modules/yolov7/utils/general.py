...
from Modules.yolov7.utils.google_utils import gsutil_getsize
from Modules.yolov7.utils.metrics import fitness
from Modules.yolov7.utils.torch_utils import init_torch_seeds
...
def check_dataset(dict):
    ...
    if val and len(val):
        ...
        if not all(x.exists() for x in val):
            print('\nWARNING: Dataset not found, nonexistent paths: %s' % [str(x) for x in val if not x.exists()], s)
